...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            int temp_err_flag = 0, line_len = strlen(line);
            char s_method[line_len], s_request_target[line_len], s_HTTP_version[line_len];

            // --- first eval
            
            for (int i = 0, j = 0, space = 0, queryflag = 0; i < line_len; i++, j++)
            {
                if(line[i] == ' ')
                {
                    switch (space)
                ...
                if (line[i] != ' ')
                {
                    switch (space)
                    {
                        ...
                        case 1:
                            s_request_target[j] = line[i];
                            break;
                        
                        case 2:
                            s_HTTP_version[j] = line[i];
                            break;       
                    }
                }  
            }
            ...
            if(s_request_target[0] != '/')
            {             
                error(501);
                continue;
            }

            if(strcmp(s_HTTP_version, "HTTP/1.1") != 0)
            {
                error(505);
                continue;
            }
            
            if(strchr(s_request_target, '.') == NULL)
            {
                error(501);
                continue;
            }
            ...
            // UPDATE:done (reminder, implement another fullstop 501 from absolute path over here)
            char query[strlen(s_request_target) + 1], s_absolute_path[strlen(s_request_target) + 1];
            char *query_position = strchr(s_request_target, '?');
            int indice = (query_position - &s_request_target[0]);
            
            if (query_position == NULL)
            {
                query[0] = '\0';
                strcpy(s_absolute_path, s_request_target);
            }
            else
            {
                for (int i = indice + 1, j = 0, k = strlen(s_request_target) + 1; i < k; i++, j++)
                {
                    query[j] = s_request_target[i];
                }
                for (int i = 0; i < indice; i++)
                {
                    s_absolute_path[i] = s_request_target[i];
                }
                s_absolute_path[indice] = '\0';
            }
            
            if(strchr(s_absolute_path, '.') == NULL)
            {
                error(501);
                continue;
            }
            ...
            char path[strlen(root) + strlen(s_request_target) + 1];
            strcpy(path, root);
            strcat(path, s_absolute_path);
            ...
            char extension[strlen(s_absolute_path) + 1];
            indice = (strchr(s_absolute_path, '.') - &s_absolute_path[0]);
            
            for (int i = indice + 1, j = 0, k = strlen(s_absolute_path) + 1; i < k; i++, j++)
            {
                extension[j] = s_absolute_path[i];
            }
            ...
            else
            {
                ...
                /*
                // look up file's MIME type (UPDATE: Modified stuff here)
                if (lookup(extension) == NULL)
                {
                    printf("FailDebug: %s from %s", lookup(extension), extension);
                    error(501);
                    continue;
                }*/
